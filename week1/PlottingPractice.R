...
url <-
        "https://d3c33hcgiwev3.cloudfront.net/_e143dff6e844c7af8da2a4e71d7c054d_payments.csv?Expires=1486425600&Signature=bVUpNT7u~gSn2UZPJ8jtAnVUerUMwbdupjjJQcr0FzX5v5f4ooJHiFbfP1uHEVxixgK0z~kzDzGT~TyiIG7aUrBM2msufeTAWWD0iyi6W6noFZzdhawJkhUEDIaJ6XoVI55b9yZeHz5DSK7rGnhLwAMmcbdps0CXPeHqtVS9fw8_&Key-Pair-Id=APKAJLTNE6QMUY6HBC5A"
...
#### readin data
dataset <-
        read.table(file = 'original_data.csv', sep = ",", header = TRUE)

#### make plot1
plot(
        dataset$Average.Covered.Charges,
        dataset$Average.Total.Payments,
...
model <-
        lm(Average.Total.Payments ~ Average.Covered.Charges, data = dataset)
...
par(
        mfrow = c(length(defi), length(state)),
        mar = c(1, 1, 1, 1),
        oma = c(0, 0, 3, 0)
)
for (x in defi) {
        for (y in state) {
                ...
                plot(
                        tempdata$Average.Covered.Charges,
